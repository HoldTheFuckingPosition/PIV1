...
use piv1_genesis_initialization_caller::{prepare, process_instruction, PreparedCall, HOST_UNAVAILABLE};

fn world(shared: bool) -> Fixture {
    let mut f=Fixture::new(); f.accounts.truncate(8);
    for index in 8..if shared {34} else {35} {
    ...
    f.accounts[payer].signer=true;f.accounts[payer].writable=true;
    f.accounts[payer+1].key=system_program::ID;f.accounts[payer+1].executable=true;
    f.accounts[payer+2].key=spl_token::ID;f.accounts[payer+2].executable=true;
...
fn approved_message_recovers_exact_inner_privileges_data_order_and_seeds() {
    for shared in [false,true] {
        ...
        assert_eq!(call.instruction.accounts.len(),if shared {34} else {35});
        assert_eq!(f.accounts[TRANSACTION].data.len(),if shared {1546} else {1579});
        let payer=if shared {29} else {30};
        for (index,meta) in call.instruction.accounts.iter().enumerate() {
            assert_eq!(meta.pubkey,f.accounts[index].key);
            assert_eq!(meta.is_signer,index==5 || index==payer);
            assert_eq!(meta.is_writable,(7..=22).contains(&index) || index==payer);
        }
...
fn raw_u32_lengths_reject_before_unbounded_indexing_or_allocation() {
    for shared in [false,true] {
        let original=world(shared);let count=if shared {34} else {35};
...
#[test]
fn payer_must_be_a_real_outer_signer_and_writable_in_both_profiles() {
    for shared in [false,true] {
        let payer=if shared {29} else {30};
        for remove_signer in [false,true] {
            let mut f=world(shared);let payer_key=f.accounts[payer].key;
            assert!(run_with(&mut f,SQUADS_V4_PROGRAM_ID,|accounts,_| {
                for account in accounts.iter_mut().filter(|a|*a.key==payer_key) {
                    if remove_signer {account.is_signer=false;} else {account.is_writable=false;}
                }
            }).is_err());
            let mut f=world(shared);
            if remove_signer {f.accounts[payer].signer=false;} else {f.accounts[payer].writable=false;}
            f.rebuild_message();assert!(run(&mut f).is_err());
        }
    }
}

#[test]
fn every_fixed_inner_privilege_is_required_and_old_topologies_reject() {
    for shared in [false,true] {
        let original=world(shared);
        for role in 0..original.accounts.len() {
            for signer in [false,true] {
                let mut f=original.clone();
                if signer {f.accounts[role].signer^=true;} else {f.accounts[role].writable^=true;}
                f.rebuild_message();assert!(run(&mut f).is_err(),"shared={shared}, role={role}, signer={signer}");
            }
        }
        for count in [31,32,33] {
            let mut f=original.clone();f.accounts.truncate(count);f.rebuild_message();
            assert!(run(&mut f).is_err(),"shared={shared}, count={count}");
        }
    }
}

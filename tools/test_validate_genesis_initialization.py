...
SCRIPT = Path('/home/jerem/piv1/tools/validate_genesis_initialization.py')
...
class Guards(unittest.TestCase):
    ...
    def test_sources_reject_symlink_before_read(self):
        with tempfile.TemporaryDirectory(prefix='piv1-t232-unit-') as temp:
    ...
    def test_fresh_path_refuses_existing_and_outside_tmp(self):
        for path in [Path('/home/piv1-genesis-initialization-runtime-test'),Path('/tmp/wrong-name')]:
            with self.assertRaisesRegex(RuntimeError, 'unexpected output'):
                runner.checked_output(path, fresh=True)
        with tempfile.TemporaryDirectory(prefix='piv1-genesis-initialization-runtime-unit-') as temp:
        ...
        with patch.object(runner,'checked_output') as checked:
            for approved in [None,'','a'*63,'z'*64]:
                with self.assertRaisesRegex(RuntimeError,'reviewed executable'):
                    runner.execute(None,None,Path('/tmp/piv1-genesis-initialization-runtime-build'),approved)
    ...
    def test_missing_third_token_artifact_rejected_before_loading_any_elf(self):
        pins={'schema':1,'baseline_head':runner.BASELINE,'sources':{},'protected_inputs':{},
              'tools':{},'aliases':{},'artifacts':{'caller':{},'callee':{}},'historical_artifacts':[]}
        class Helper:
            @staticmethod
            def verify_artifact(_):raise AssertionError('no ELF read allowed')
        with patch.object(runner,'digest',return_value=runner.HELPER_SHA256),patch.object(runner,'sources',return_value={}):
            with self.assertRaisesRegex(RuntimeError,'all three exact'):
                runner.verify_inputs(Helper,pins)

    def test_changed_third_token_artifact_rejected(self):
        pins={'schema':1,'baseline_head':runner.BASELINE,'sources':{},'protected_inputs':{},
              'tools':{},'aliases':{},'artifacts':{'caller':'caller','callee':'callee','token':'token'},'historical_artifacts':[]}
        seen=[]
        class Helper:
            @staticmethod
            def verify_artifact(artifact):
                seen.append(artifact)
                if artifact=='token':raise RuntimeError('exact Token artifact drift')
        with patch.object(runner,'digest',return_value=runner.HELPER_SHA256),patch.object(runner,'sources',return_value={}):
            with self.assertRaisesRegex(RuntimeError,'Token artifact drift'):
                runner.verify_inputs(Helper,pins)
        self.assertEqual(seen,['caller','callee','token'])

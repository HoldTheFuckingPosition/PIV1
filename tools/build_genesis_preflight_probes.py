...
SCRIPT = ROOT / 'tools/build_genesis_preflight_probes.py'
PINS = ROOT / 'tools/genesis_preflight_probe_pins.json'
WORKSPACE = ROOT / 'validation/genesis-preflight-probes'
...
PACKAGES = ('piv1-genesis-preflight-probe', 'piv1-genesis-preflight-caller')
...
def probe_sources(helper):
    paths = [SCRIPT, ROOT / 'tools/test_build_genesis_preflight_probes.py']
...
def reject_configs(helper):
    ...
    for directory in [WORKSPACE, WORKSPACE / 'callee', WORKSPACE / 'caller', WORKSPACE.parent,
                      helper.CARGO_HOME]:
...
def verify_lock(pins):
    ...
    require(sorted((p['name'], p['version']) for p in probe['package'] if 'source' not in p)
            == [('piv1', '0.1.0'), ('piv1-genesis-preflight-caller', '0.1.0'),
                ('piv1-genesis-preflight-probe', '0.1.0'), ('piv1-math', '0.1.0')], 'unexpected local package')
    return records
...
def verify_inputs(helper, pins):
    ...
    production = json.loads(PRODUCTION_PINS.read_text())
...
def main():
    ...
    require(output.parent == Path('/tmp') and re.fullmatch(r'piv1-genesis-probes-[a-z0-9-]+', output.name), 'unexpected output path')

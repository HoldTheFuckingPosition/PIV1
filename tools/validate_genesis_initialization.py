#!/usr/bin/python3
"""Locked/offline three-ELF full-initialization validation; no rebuild or live operation.
...
WORKSPACE = ROOT / 'validation/genesis-initialization-runtime'
SCRIPT = ROOT / 'tools/validate_genesis_initialization.py'
PINS = ROOT / 'tools/genesis_initialization_runtime_pins.json'
HELPER = ROOT / 'tools/validate_sbf_claims.py'
HELPER_SHA256 = 'ff4706b6101b57c6385b0ae43b175a3ed252cb40e150f7a3120a84beb7c01fc8'
BASELINE = '60193d63b64f42211f98d9b4910dfc047ab876df'
...
def sources():
    paths = [SCRIPT, ROOT / 'tools/test_validate_genesis_initialization.py']
...
def verify_lock(pins):
    ...
    require(sorted((p['name'], p['version']) for p in current['package'] if 'source' not in p)
            == [('piv1', '0.1.0'), ('piv1-genesis-initialization-runtime', '0.1.0'), ('piv1-math', '0.1.0')], 'unexpected local package')
...
def verify_inputs(helper, pins):
    ...
    require(set(pins['artifacts']) == {'caller', 'callee', 'token'}, 'all three exact probe artifacts required')
...
def checked_output(path, fresh=False):
    require(path.parent == Path('/tmp') and re.fullmatch(r'piv1-genesis-initialization-runtime-[a-z0-9-]+', path.name), 'unexpected output path')

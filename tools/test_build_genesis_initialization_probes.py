...
SCRIPT = Path('/home/jerem/piv1/tools/build_genesis_initialization_probes.py')
...
class ProbeGuards(unittest.TestCase):
    def setUp(self):
        self.root = Path(tempfile.mkdtemp(prefix='piv1-genesis-initialization-probes-tests-'))
    ...
    def test_only_exact_expat_adjustment_is_derived_without_mutating_historical_pins(self):
        before=runner.PRODUCTION_PINS.read_bytes()
        original=json.loads(before)
        profile=runner.current_target_profile({'target_profile_adjustment':dict(runner.PROFILE_ADJUSTMENT)})
        self.assertEqual(profile['files'][runner.PROFILE_ADJUSTMENT['path']]['sha256'],
                         runner.PROFILE_ADJUSTMENT['new_sha256'])
        profile['files'][runner.PROFILE_ADJUSTMENT['path']]['sha256']=runner.PROFILE_ADJUSTMENT['old_sha256']
        self.assertEqual(profile,original)
        self.assertEqual(runner.PRODUCTION_PINS.read_bytes(),before)
        for field in ('path','resolved','old_sha256','new_sha256'):
            change=dict(runner.PROFILE_ADJUSTMENT);change[field]='unreviewed'
            with self.assertRaisesRegex(RuntimeError,'unsupported target profile adjustment'):
                runner.current_target_profile({'target_profile_adjustment':change})
        changed=self.root/'old-pins.json';changed.write_bytes(before+b' ')
        with patch.object(runner,'PRODUCTION_PINS',changed):
            with self.assertRaisesRegex(RuntimeError,'production pin drift'):
                runner.current_target_profile({'target_profile_adjustment':runner.PROFILE_ADJUSTMENT})
        for field in ('sha256','resolved','reviewed_tools'):
            invalid=json.loads(before);path=runner.PROFILE_ADJUSTMENT['path']
            if field=='reviewed_tools':invalid[field][path]=runner.PROFILE_ADJUSTMENT['old_sha256']
            else:invalid['files'][path][field]='unreviewed'
            changed.write_text(json.dumps(invalid))
            with patch.object(runner,'PRODUCTION_PINS',changed), \
                    patch.object(runner,'PRODUCTION_PINS_SHA256',runner.digest(changed)):
                with self.assertRaisesRegex(RuntimeError,'historical file identity|original reviewed tool'):
                    runner.current_target_profile({'target_profile_adjustment':runner.PROFILE_ADJUSTMENT})
    ...
    def test_configuration_and_credentials_refuse_without_reading_payload(self):
        ...
        for directory, name in [(workspace/'caller', '.cargo/config.toml'), (workspace/'token', '.cargo/config'),
    ...
    def test_probe_source_inventory_includes_nested_crates_and_rejects_symlinks(self):
        ...
        (self.root/'tools').mkdir();(self.root/'tools/test_build_genesis_initialization_probes.py').write_text('test')
        helper=runner.load_helper()
        with patch.object(runner,'WORKSPACE',workspace),patch.object(runner,'ROOT',self.root),patch.object(runner,'SCRIPT',self.root/'tools/test_build_genesis_initialization_probes.py'):
    ...
    def simulated_run(self, *, stderr='', malformed_elf=False, token_elf_failure=False, audit_failure=False, package_failure=False):
        ...
        pin.write_text(json.dumps({'schema':1,'baseline_head':'reviewed','packages':[],
                                   'target_profile_adjustment':runner.PROFILE_ADJUSTMENT}))
        ...
        with contextlib.ExitStack() as stack:
            ...
            if token_elf_failure:
                def inspect(path):
                    if path.name=='piv1_genesis_initialization_token.so':raise struct.error('rejected Token ELF')
                    return {'sha256':'mock-only','bytes':9}
                stack.enter_context(patch.object(helper,'elf_summary',side_effect=inspect))
    ...
    def test_third_token_artifact_is_required_after_both_other_artifacts(self):
        result=self.simulated_run(token_elf_failure=True)
        self.assertEqual(result['error'],'rejected Token ELF')
        self.assertEqual(set(result['artifacts']),{'piv1-genesis-initialization-probe','piv1-genesis-initialization-caller'})
        self.assertIn('piv1-genesis-initialization-token-elf.stdout',result['logs'])
        self.assertIn('piv1-genesis-initialization-token-disassembly.stdout',result['logs'])
